...
import argparse
import sys
sys.path.append("/home/zza/VScodeprogram/LawLLM/agents-master/src/")
sys.path.append("home\\zza\\VScodeprogram\\LawLLM\\agents-master\\examples\\Gradio_config")
from agents.SOP import SOP
from agents.Agent import Agent
from agents.Environment import Environment
from agents.Memory import Memory
from gradio_base import Client
from run_gradio import JudgeUI
...
def prepare(agents, sop, environment):
    ...
    content = sop.states['state1'].begin_query
    parse_data = JudgeUI.extract(content)
    client.send_message(
        {
            "theme": f"{parse_data[0]}",
            "plaintiff": f"{parse_data[1]}",
            "defendant": f"{parse_data[2]}",
            "agents_name": JudgeUI.convert2list4agentname(sop)[0],
            "only_name":  JudgeUI.convert2list4agentname(sop)[0],
    ...
    sop.states['state2'] = sop.states['state1'].begin_query = \
        JudgeUI.merge(
            theme=Client.cache["theme"], plaintiff=Client.cache["plaintiff"], defendant=Client.cache["defendant"],
            origin_content=sop.states['state1'].begin_query
        )


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='A demo of chatbot')
    parser.add_argument('--agent', type=str, help='path to SOP json', default="configMe0.json")
